...
> 版本：v1.1 · 与 `PROJECT_CHARTER.md` v1.1 对齐 · 最近更新：2026-08-18
> 本文件定义 1.0 交付物（＝19 条锁定基线全部完成）的"长什么样、怎么验、怎么回滚"。其中 §三.6 的 57 核全表（item 0）属 1.0 必备基线（**非独立 vFinal**）；v1.0＝19 条全部完成＋用户真机验证通过。
> **铁律级需求见 `PROJECT_CHARTER.md` §零（19 条 · 2026-08-18 锁定基线）；item 0/14 锁定。**
...
- **测试版 `v0.x`（含 `v0.0`）**：交付给用户真机测试用的中间版本。**版本号保持 `0.0` 直到完整版（含 57 核全表）首次交付测试**（用户 2026-08-18 明示）。已发：`v0.1` = 首个全绿构建（`05ee252dfe`）/ `v0.2` = 首个含完整门禁的绿构建（`377ac86220`）。
- **正式版 `v1.0`（＝19 条基线完成 + 用户真机验证通过）**：第一个**19 条锁定基线（items 0–18）全部完成且用户真机全部验证通过**的版本。范围涵盖 item 0（57 核全表 + LDFLAGS_S + 子模块）+ item 14（11.png 目录）+ item 15–18（启动安全 / ABI 门禁 / 策略存档 / 全手柄）。**这是唯一正式版，不存在独立的「vFinal」。** 判定：SD 卡插入 → 进入自研系统 → 9 分类（000–008）各至少 1 游戏可启动 → 跑 30 秒 → 退出 → 存档 → 读档 → 切下一个 → 全手柄可用 → 57 核各取 1 ROM 实测通过。
...
> **关键澄清（2026-08-18 用户纠正）**：**不存在独立于 1.0 之外的「vFinal」。** 19 条需求（含 item 0 的 57 核全表）是完整交付基线，全部必须完成；完成全部 19 条 + 用户真机全部验证通过后，版本号方可标记为 `v1.0`。1.0 之前一律为 `v0.x`（含 `0.0`）。
...
- [ ] `picoarch` 用 `platform=unix` 编译，链接 `-lasound`
- [ ] 注入 `-DSCREEN_WIDTH=1280 -DSCREEN_HEIGHT=720 -DSCREEN_BPP=2` 生效
- [ ] `plat_sdl.c` MStar 块**对 RK3036G 不可见**（用 PLATFORM_MSTAR 守卫；未定义时走纯 SDL）
- [ ] 视频：SDL fbdev 打开成功，输出到 HDMI（分辨率按 §四决策）
...
> **更正（2026-08-18 用户纠正）**：本节不是"1.0 之后"的额外阶段，而是 **1.0 必备基线的一部分**（item 0 锁定的 57 核全表 + LDFLAGS_S + 子模块）。19 条基线全部完成 + 用户真机全部验证通过后，方可标记 `v1.0`（不存在独立 vFinal）。
