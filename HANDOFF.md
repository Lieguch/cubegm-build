...
> 最近更新：2026-08-16（STAGE3 picoarch SDL.h 包含路径修复已推送 210c798a，run36 待验证）
...
## 9. 下一步（接手后）
- 等 run36（210c798a）结果：若 STAGE3 过 → picoarch 编出 → 进 FrogUI/核心/ABI 门禁 → 全绿打 v0.1（测试版）。
- 若仍失败：拉 build-output 日志按 §6 定位，按 §8 铁律修，再推。
- 全绿后真机验收：SRAM 落 .srm、RTC、evdev 手柄、显示。
