#!/bin/bash
# RK3036G (ARM) multicore builder -- ported from treefrog-ui build_all.sh (MIPS) — produces libretro .so files for picoarch
...
#   .toolchain/arm-gcc/g++   : full RK3036G flags (armv7-a/neon/hard-float, sysroot, Ofast) — most cores
...
MIPS="${CROSS_COMPILE:-arm-linux-gnueabihf-}"
SYSROOT="${SYSROOT:?set by build.sh}"
OUT="$(pwd)/build"
CORES="$(pwd)/cores"
WRAP="$(pwd)/.toolchain"
...
# Tune for cortex-a7 and enable NEON.
...
SF3000_FLAGS="$ARM_FLAGS"

cat > "$WRAP/arm-gcc" <<EOF
#!/bin/bash
...
# forces little-endian: this toolchain's g++ defaults to big-endian (gcc to
...
cat > "$WRAP/arm-gcc-O3" <<EOF
#!/bin/bash
exec ${MIPS}gcc $SF3000_FLAGS "\$@" -O3
EOF
cat > "$WRAP/arm-g++-O3" <<EOF
#!/bin/bash
...
#  NO          the DSP ASE codegen makes FBA's fixed-point sound/blit emit
...
# strict aliasing.
FBA_FLAGS="$(echo "$SF3000_FLAGS" | sed 's///')"
...
EOF
chmod +x "$WRAP"/arm-gcc "$WRAP"/arm-g++ "$WRAP"/gpsp-gcc "$WRAP"/gpsp-g++ \
         "$WRAP"/arm-gcc-O3 "$WRAP"/arm-g++-O3 "$WRAP"/fba-gcc "$WRAP"/fba-g++

AR="${MIPS}ar"
RANLIB="${MIPS}ranlib"
STRIP="${MIPS}strip"
LDFLAGS="-march=armv7-a -mfpu=neon-vfpv4 -mfloat-abi=hard --sysroot=$SYSROOT -L$SYSROOT/usr/lib -lm -lc -lstdc++"
LDFLAGS_C="-march=armv7-a -mfpu=neon-vfpv4 -mfloat-abi=hard --sysroot=$SYSROOT -L$SYSROOT/usr/lib -lm -lc"
...
LDFLAGS_SC="-shared -Wl,--no-undefined -march=armv7-a -mfpu=neon-vfpv4 -mfloat-abi=hard --sysroot=$SYSROOT -L$SYSROOT/usr/lib -lm -lc"
...
_b snes9x2005_plus   snes9x2005                "" "CC=$WRAP/arm-gcc-O3 CXX=$WRAP/arm-g++-O3"
...
# gpsp: pre-cpp branch, platform=armv7-neon-hardfloat — tzubertowski's dedicated target with
...
make -C "$CORES/gpsp" platform=armv7-neon-hardfloat \
...
make -C "$CORES/gpsp_upstream" platform=armv7-neon-hardfloat \
    CC="$WRAP/gpsp-gcc" CXX="$WRAP/gpsp-g++" \
...
if [ -d "$CORES/TIC-80" ]; then
    ...
    if command -v "$CMAKE" >/dev/null; then
        ...
        cat > /tmp/tic80_arm.cmake <<EOF
set(CMAKE_SYSTEM_NAME Linux)
set(CMAKE_SYSTEM_PROCESSOR arm)
set(CMAKE_C_COMPILER   ${MIPS}gcc)
set(CMAKE_CXX_COMPILER ${MIPS}g++)
set(CMAKE_SYSROOT ${SYSROOT})
set(FL "-march=armv7-a -mtune=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=hard -mlong-calls")
...
EOF
        ...
        "$CMAKE" -S "$CORES/TIC-80" -B /tmp/tic80_build \
            -DCMAKE_TOOLCHAIN_FILE=/tmp/tic80_arm.cmake -DCMAKE_BUILD_TYPE=Release \
...
(
  ...
  ADFLAGS="-march=armv7-a -mtune=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=hard --sysroot=$SYSROOT -fPIC -Ofast -fomit-frame-pointer -DNDEBUG"
  ...
  if [ -d "$AD" ]; then
    ...
    if [ "$ok" = 1 ]; then
      ${MIPS}g++ --sysroot=$SYSROOT -fPIC -shared -Wl,--version-script="$AD/link.T" $AOBJ -o "$AD/arduous_libretro.so" -lc -lm && \
...
(
  ...
  if [ -d "$AR_DIR" ]; then
    ...
    ARARCH="-march=armv7-a -mtune=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=hard --sysroot=$SYSROOT -fPIC -O3 -fomit-frame-pointer -DNDEBUG"
...
_b fake08            fake-08/platform/libretro "" "CC=$WRAP/arm-gcc-O3 CXX=$WRAP/arm-g++-O3"
...
make -C "$CORES/pcsx_rearmed" -f Makefile.libretro platform=unix \
    CC="$WRAP/arm-gcc" CXX="$WRAP/arm-g++" CC_AS="$WRAP/arm-gcc" CC_LINK="$WRAP/arm-g++" \
    AR="$AR" ARCH=arm DYNAREC=lightrec HAVE_NEON=1 BUILTIN_GPU=unai -j"$(nproc)" 2>&1
...
# the toolchain defaults to big-endian — the in $WRAP fixes that.
echo "-- frodo make --"
...
make -C "$CORES/libretro-frodo" platform=unix NOLIBCO=1 \
    CC="$WRAP/arm-gcc" CXX="$WRAP/arm-g++" \
    AR="$AR" RANLIB="$RANLIB" LD="$WRAP/arm-g++" \
